...
This is the original implmentation from that source. It slices the sequences
to calculate the common sequences. This results in using more memory and
may affect peformance.

The only significant change from the original sources is the use ofNumPy arrays
when calculating the common sequences, for faster performance and save memory.
...
def _lcs_lens(xs, ys):
    ...
    # print('   xs: {} ys: {} curr: {}'.format(xs, ys, curr))
    return curr
...
def lcs(xs, ys):
    ...
    if nx == 0:
        ...
        yb, ye = ys[:k], ys[k:]
        # print('     k:{} cost:{} yb:{} ye:{}'.format(k, cost, yb, ye))
